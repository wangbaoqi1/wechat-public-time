...
export const buildTianApi = async (apiType, params = null) => {
  ...
  if (RUN_TIME_STORAGE[`${apiType}`]) {
    return RUN_TIME_STORAGE[`${apiType}`]
  }
  const url = `https://apis.tianapi.com/${apiType}/index?key=${config.TIAN_API.key}`

  const res = await axios.get(url).catch((err) => err)
  if (res && res.data && res.data.code === 200) {
    const result = res.data.result.content

    RUN_TIME_STORAGE[`${apiType}`] = cloneDeep(result)
    return result
  }
...
export const getAggregatedData = async () => {
  ...
  // 获取土味情话
  const earthyLoveWords =
    (await getEarthyLoveWords()) || DEFAULT_OUTPUT.earthyLoveWords
  // 获取朋友圈文案
  const momentCopyrighting =
    (await getMomentCopyrighting()) || DEFAULT_OUTPUT.momentCopyrighting
  // 获取毒鸡汤
  const poisonChickenSoup =
    (await getPoisonChickenSoup()) || DEFAULT_OUTPUT.poisonChickenSoup
  // 获取古诗古文 poetry
  const {
  ...
  for (const user of users) {
    ...
    console.log('tianApiGreeting', tianApiGreeting)
    // 天行-天气
    ...
    //节假日提醒
    const holidaty = await getHolidaty()
    console.log('holidaty', holidaty)
    const holidatyArr =
      (!!holidaty &&
        Object.keys(holidaty).map((i) => ({
          name: `holiday_${i}`,
          value: holidaty[i],
        }))) ||
      []
    ...
    const wxTemplateParams = [
      ...
      {
        name: toLowerLine('noteEn'),
        value: noteEn,
        // color: getColor()
      },
      {
        name: toLowerLine('noteCh'),
        value: noteCh,
        // color: getColor()
      },
      {
        name: toLowerLine('oneTalk'),
        value: oneTalk,
        // color: getColor()
      },
      {
        name: toLowerLine('talkFrom'),
        value: talkFrom,
        //  color: getColor()
      },
      {
...
const getHolidaty = async () => {
  let holidaty
  let holidatyObj
  if (RUN_TIME_STORAGE.holidaty) {
    console.log('获取了相同的数据，读取缓存 >>> accessToken')
    holidaty = RUN_TIME_STORAGE.holidaty
  } else {
    const url = `https://apis.tianapi.com/jiejiari/index? ${obj2strUrl({
      key: '4dfe1231571432215c664649a09176e2',
      type: 1,
    })}`
    const res = await axios.get(url).catch((err) => err)
    const data = (res?.data?.result?.list || []).map((i) => {
      return {
        tip: i.tip,
        rest: i.rest,
        name: i.name,
        startData: +selfDayjs(i?.vacation.split('|')[0]),
      }
    })
    RUN_TIME_STORAGE.holidaty = data
    holidaty = data || []
  }

  holidaty.forEach((item) => {
    holidatyObj = undefined
    if (
      (item.startData - +selfDayjs()) / 1000 / 60 / 60 / 24 < 6 &&
      (item.startData - +selfDayjs()) / 1000 / 60 / 60 / 24 > 0
    ) {
      item.diffTime = (item.startData - +selfDayjs()) / 1000 / 60 / 60 / 24
      holidatyObj = item
    }
  })
  console.log(' holidaty.forEach', holidaty, holidatyObj)
  return holidatyObj
}
 ...
 * @returns {Promise<{success: boolean, name}>}
 */
export const sendMessage = async (templateIds, user, params, usePassage) => {
...
export const sendMessageReply = async (
  ...
  for (const user of users) {
    ...
    resList.push(
      await sendMessage(
        templateId || user.useTemplateIds,
        user,
